...
{//[POST]: 
	...
	$s = mysqli_query($conn, "INSERT INTO `ENJOY` (`ID`, `USER`, `LISTING`)
	VALUES (NULL, '{$USER_ID}', '{$request['listing']}');");	
	
	///////////// ERROR REPORTING ///////////////
	mysqli_query($conn,"INSERT INTO `ERROR_REPORTING` (`ID`,`USER`, `DESCRIPTION`, `RESPONSE`,`TIMESTAMP`)VALUES (NULL, '". $USER_ID ."', '[{$_SERVER['PHP_SELF']}] [GET] Failed to post an enjoy mark.', '". str_replace("'","`",mysql_error()) ."'," . time() . ");");
