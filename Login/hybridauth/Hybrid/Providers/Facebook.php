...
class Hybrid_Providers_Facebook extends Hybrid_Provider_Model
{
	...
	public $scope = "email, user_about_me, user_birthday, user_hometown, user_website, read_stream, offline_access, publish_stream, read_friendlists, user_groups, user_photos";
	...
	*/
	function setUserStatus( $status )
	{
		...
		$parameters["message"] = "Synclis Allows you to post your listings similarly to Craigslist seamlessly to Facebook, Twitter, and our network of listings. It’s Free to use Synclis, so why not get started today?"; 
		$parameters["link"] = $status[0];
		$parameters["source"] = $status[1];
		$parameters["name"] = $status[2];
		$parameters["caption"] = "Click to view my listing on Synclis.";
		$parameters["description"] = str_replace($tags, " ", $status[3]);


		try{ 
			$response = $this->api->api( $status[4] ."/feed", "post", $parameters );
		}
		catch( FacebookApiException $e ){
	...
	*/
	...
	function getUserGroups()
	{
		try{
			...
			throw new Exception( "User group stream request failed! {$this->providerId} returned an error: $e" );
		} 
